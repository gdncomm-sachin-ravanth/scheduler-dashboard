...
The `fetch_scheduler_data.py` script automatically fetches scheduler data from the API and generates the required JSON files for the dashboard.

### Description

This script:
- Executes the curl command from `curl-command.txt` to fetch scheduler data from the API
- Extracts all records from the API response and saves them to `past-data.json`
- Filters records for today's date (using WIB timezone, UTC+7) and saves them to `today-data.json`
...
```bash
python3 fetch_scheduler_data.py
```

The script will:
1. Execute the curl command from `curl-command.txt`
2. Save all fetched records to `past-data.json`
3. Filter and save today's records to `today-data.json`
4. Display progress information including record counts

### Output Files

- `past-data.json` - Contains all scheduler execution records from the API (last 14 days)
...
1. Ensure all three JSON data files are in the same directory as `scheduler-dashboard.html`:
   ...
   - `today-data.json` - Today's execution results (generated by `fetch_scheduler_data.py`)
   - `past-data.json` - Historical execution data (generated by `fetch_scheduler_data.py`)
