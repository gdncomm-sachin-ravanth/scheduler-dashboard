...
- `.validation_cache/today-data.json` - Contains only records with `analyticDate` matching today's start timestamp in WIB timezone (TTL: 1 day)
...
3. Use the **🔄 Refresh Data** button in the top-right corner to update data without leaving the dashboard
...
1. Ensure all JSON data files are available:
   - `jobs-data.json` - Rundeck job configurations (in root directory)
   - `.validation_cache/today-data.json` - Today's execution results (generated by `fetch_today_data.py`, TTL: 1 day)
   - `.validation_cache/past-data.json` - Historical execution data (must exist for Reports sections to work, TTL: 1 day)
